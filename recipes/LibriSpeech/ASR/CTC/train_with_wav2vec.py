...
class ASR(sb.Brain):
    ...
    def fit(
        ...
        for epoch in epoch_counter:
            ...
            # Validation stage if early stopping is enforced
            if early_stopping:
                ...
                logger.info(f"Validation loss: {avg_valid_loss}")
                if avg_valid_loss >= self.previous_avg_loss:
                    self.epochs_no_improvement += 1
                    if self.epochs_no_improvement >= max_epochs_no_improvement:
                        logger.info(f"Stopping at epoch {epoch_counter.current}, no improvement in validation" +
                                    f" loss since {max_epochs_no_improvement} epoch(s)")
                        break
                
                else:
                    # Only save the model when it reaches a new best validation loss value
                    if avg_valid_loss < self.best_avg_loss:
                        self.checkpointer.save_and_keep_only()
                        self.best_avg_loss = avg_valid_loss
                    self.epochs_no_improvement = 0
                    self.previous_avg_loss = avg_valid_loss
...
if __name__ == "__main__":
    ...
    asr_brain.fit(
        ...
        max_epochs_no_improvement = hparams["max_epochs_no_improvement"]
    )
